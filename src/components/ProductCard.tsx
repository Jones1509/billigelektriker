  ...
  return (
    <Card className="group overflow-hidden transition-all duration-300 hover:shadow-xl hover:-translate-y-1 border border-border rounded-2xl h-full flex flex-col">
      <Link to={`/product/${node.handle}`} className="relative block">
        <div className="relative h-[120px] sm:h-[220px] overflow-hidden bg-muted/30 rounded-t-2xl">
          {imageUrl ? (
            <>
              <img
                src={imageUrl}
      ...
      <CardContent className="p-2 sm:p-3.5 flex-1 flex flex-col">
        <Link to={`/product/${node.handle}`}>
          <h3 className="font-semibold text-[11px] sm:text-base leading-tight mb-1 sm:mb-1.5 line-clamp-2 group-hover:text-primary transition-colors duration-200">
            {node.title}
          </h3>
        </Link>
